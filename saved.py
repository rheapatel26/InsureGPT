...
# Define file paths
terms_file_path = os.path.join(os.getcwd(), "data/Important_terms.xlsx")
...
def ask():
    try:
        ...
        if term_definition:
            return jsonify({'response': term_definition, 'type': 'term'})

        # If not an important term, search in FAQs
        faq_answers = semantic_search(corrected_query)
        if faq_answers:
            return jsonify({'response': faq_answers, 'type': 'faq'})
        ...
        return jsonify({'response': qa_answer if qa_answer else "Sorry, I couldn't find an answer to your question.", 'type': 'qa'})
...
# no rephrasing but with faqs and terms both-------------------------------------------------------------------------------
from flask import Flask, request, jsonify, render_template
import pandas as pd
import os
from transformers import pipeline
